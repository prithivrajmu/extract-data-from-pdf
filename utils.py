...
def validate_pdf_file(file) -> Tuple[bool, str]:
    ...
    if file_size_mb > max_size_mb:
        return (
            False,
            f"File '{file.name}' is too large ({file_size_mb:.2f} MB). Maximum size is {max_size_mb} MB.",
        )
    ...
    if len(file_bytes) < 4 or not file_bytes.startswith(b"%PDF"):
        return (
            False,
            f"File '{file.name}' does not appear to be a valid PDF (missing %PDF header).",
        )
    ...
    if b"%%EOF" not in file_bytes[-2048:]:
        return (
            False,
            f"File '{file.name}' appears to be incomplete (missing %%EOF marker).",
        )
...
def save_results_to_file(
    df: pd.DataFrame, output_path: str, format: str = "csv"
) -> bool:
    """
...
def dataframe_to_json_string(
    ...
    if format_lower == "standard":
        ...
        raise ValueError(
            f"Unknown JSON format: {format}. Use 'standard', 'structured', 'multi_file', or 'unified'."
        )
...
def create_structured_json_output(
    ...
    output = {"metadata": final_metadata, "data": data}
...
def create_multi_file_json(df: pd.DataFrame, metadata: Optional[Dict] = None) -> str:
    ...
    output = {**file_data, "metadata": final_metadata}
...
def create_unified_json(df: pd.DataFrame, metadata: Optional[Dict] = None) -> str:
    ...
    for record in records:
        unified_record = {**record, "_metadata": final_metadata}
