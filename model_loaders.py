...
def load_chandra_model(pdf_path: str, output_dir: str, use_cpu: bool = False) -> Tuple[str, Optional[Dict]]:
    """
...
def is_model_supported(model_name: str) -> Tuple[bool, str]:
    ...
    if any(chandra in model_name.lower() for chandra in chandra_models):
        ...
        try:
            subprocess.run(['chandra', '--version'], 
                         capture_output=True, check=True, timeout=5)
            return True, "Fully supported via Chandra CLI"
        except (subprocess.CalledProcessError, FileNotFoundError, subprocess.TimeoutExpired):
            return False, "Chandra CLI not installed. Install with: pip install chandra-ocr"
    
    elif any(tf in model_name.lower() for tf in transformers_models):
        return True, "Supported via transformers library (may need additional dependencies)"
    
    else:
        return False, f"Model '{model_name}' needs custom implementation. We can add support - please file an issue!"
