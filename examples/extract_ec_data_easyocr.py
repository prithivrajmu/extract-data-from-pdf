...
try:
    ...
    print("💡 Tip: Install fuzzywuzzy for better name matching: pip install fuzzywuzzy python-Levenshtein")
...
def extract_document_number_fuzzy(row_text: str) -> str:
    ...
    for keyword in keywords:
        keyword_match = re.search(rf'{keyword}[^0-9]*?(\d{{2,6}}[/\-]\d{{4}})', row_text, re.IGNORECASE)
        if keyword_match:
...
def extract_plot_numbers_fuzzy(row_text: str, row_lines: List[str]) -> str:
    ...
    village_patterns = [
        ...
        re.compile(r'Vilage[^&]*&[^,]*?((?:\d+[/\-]\d+[,\s]*)+)', re.IGNORECASE),  # OCR typo
        re.compile(r'VIlage[^&]*&[^,]*?((?:\d+[/\-]\d+[,\s]*)+)', re.IGNORECASE),  # OCR typo
    ]
            ...
            if ',' in line and '/' in line:
                ...
                for plot in matches:
                    ...
                    if len(parts) == 2 and parts[0].isdigit() and parts[1].isdigit():
                        ...
                        if len(parts[1]) <= 3 and 1 <= int(parts[0]) <= 9999 and 1 <= int(parts[1]) <= 999:
                            plot_numbers.append(normalized)
        ...
        for plot in all_matches:
            ...
            if len(parts) == 2 and parts[0].isdigit() and parts[1].isdigit():
                ...
                if len(parts[1]) <= 3 and 1 <= int(parts[0]) <= 9999 and 1 <= int(parts[1]) <= 999:
                    ...
                    plot_context = row_text[max(0, row_text.find(plot)-20):row_text.find(plot)+50]
                    if 'village' in plot_context.lower() or 'schedule' in plot_context.lower() or 'plot' in plot_context.lower():
                        plot_numbers.append(normalized)
...
def extract_names_fuzzy(row_text: str, row_lines: List[str]) -> Tuple[str, str]:
    ...
    for keyword in executant_keywords:
        keyword_match = re.search(rf'{keyword}[:\s]+([A-Z][A-Za-z\s]+{{2,20}})', row_text, re.IGNORECASE)
        if keyword_match:
    ...
    valid_names = []
    exclude_keywords = ['plot', 'survey', 'document', 'village', 'schedule', 'date', 
                       'registration', 'nature', 'page', 'vol', 'no', 'year', 'pr',
                       'consideration', 'value', 'august', 'june', 'december', 'january']
    ...
    for name in all_name_candidates:
        ...
        if not any(keyword in name_lower for keyword in exclude_keywords):
            if len(name.split()) >= 2 and len(name) > 5:
                ...
                if not re.match(r'^[IVX]+$', name) and not re.match(r'^\d+[/-]\d+', name):
...
def parse_table_rows(text: str) -> List[Dict[str, str]]:
    ...
    plot_no_pattern = re.compile(r'(?:Plot\s+No[./:\s]*|Village[^&]*&[^,]*|Schedule[^,]*)?([0-9]+/[0-9]+)', re.IGNORECASE)
    
    # Survey No: patterns like "Survey 103/3" or "103/3" near Survey
    survey_pattern = re.compile(r'Survey[^0-9]*([0-9]+/[0-9]+)|^([0-9]+/[0-9]+).*Survey', re.IGNORECASE)
    ...
    for line in lines[:50]:  # Check first 50 lines
        if 'Survey' in line:
            ...
            survey_header_match = re.search(r'Survey[^0-9]*([0-9]+/[0-9]+)', line, re.IGNORECASE)
    ...
    while i < len(lines):
        ...
        if serial_match:
            ...
            if len(serial_no) == 4 and int(serial_no) >= 1900 and int(serial_no) <= 2100:
            ...
            for j in range(i + 1, min(i + 30, len(lines))):
                ...
                if j > i + 2:  # Give some buffer
                    if re.match(r'^\s*\d+\s*$', next_line):
                        ...
                        if re.search(r'\d{2,6}/\d{4}', lookahead):  # Has document number
                            break
                    
                    # Also stop if we see a pattern like "X" where X is next serial
                    if re.match(r'^\s*\d+\s+', next_line):
                        next_serial = re.match(r'^\s*(\d+)\s+', next_line).group(1)
                        if next_serial.isdigit() and int(next_serial) == int(serial_no) + 1:
                            break
            ...
            current_row['Document No.& Year'] = extract_document_number_fuzzy(row_text_normalized)
            
            # Extract Plot No using fuzzy matching
            current_row['Plot No'] = extract_plot_numbers_fuzzy(row_text_normalized, row_lines)
            
            # Extract Survey No using fuzzy matching
            current_row['Survey No'] = extract_survey_number_fuzzy(row_text_normalized, survey_no_header)
