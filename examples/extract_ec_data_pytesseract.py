...
def parse_table_rows(text: str) -> list[dict[str, str]]:
    ...
    while i < len(lines):
        ...
        elif "Document No.& Year" not in current_row or not current_row.get(
            "Document No.& Year"
        ):
        ...
        plot_match = re.search(
            r"(?:Plot\s*No[\.:]?\s*)([A-Z0-9/]+)", line, re.IGNORECASE
        )
        if plot_match:
        ...
        survey_match = re.search(
            r"(?:Survey\s*No[\.:]?\s*)([A-Z0-9/]+)", line, re.IGNORECASE
        )
        if survey_match:
        ...
        if "Claimant" in line or "claimant" in line.lower():
            name_match = re.search(
                r"(?:Claimant[\(s\)]*[:\-]?\s*)(.+)", line, re.IGNORECASE
            )
            if name_match:
