        ...
        for retry in range(max_retries):
            try:
                ...
                if retry < max_retries - 1:
                    wait_time = (retry + 1) * 5
                    print(
                        f"   ⚠️  Upload failed (attempt {retry + 1}/{max_retries}): {e}"
                    )
                    print(f"   Waiting {wait_time} seconds before retry...")
                    time.sleep(wait_time)
                else:
                    raise Exception(
                        f"Failed to upload PDF after {max_retries} attempts: {e}"
                    )
        ...
        for retry in range(max_retries):
            try:
                try:
                    ...
                    if "model name format" in str(gen_error).lower() or "400" in str(
                        gen_error
                    ):
                        print(
                ...
                if (
                    ...
                    if retry < max_retries - 1:
                        ...
                        print(
                            f"   ⚠️  Rate limit hit (attempt {retry + 1}/{max_retries})"
                        )
                        print(f"   Waiting {wait_time} seconds before retry...")
                        time.sleep(wait_time)
                        continue
                ...
                if retry == max_retries - 1:
                    raise Exception(
                        f"Failed to extract data after {max_retries} attempts: {e}"
                    )

                wait_time = (retry + 1) * 5
                print(
                    f"   ⚠️  Extraction failed (attempt {retry + 1}/{max_retries}): {e}"
                )
                print(f"   Waiting {wait_time} seconds before retry...")
                time.sleep(wait_time)
...
def extract_data_from_pdf_gemini(
    pdf_path: str,
    model: genai.GenerativeModel,
    max_retries: int = 3,
    model_name: str = None,
    ...
    try:
            ...
            try:
                ...
                if (
                    ...
                    if retry < max_retries - 1:
                        wait_time = (
                            retry + 1
                        ) * 10  # Longer wait for rate limits: 10s, 20s, 30s
                        print(
                            f"   ⚠️  Rate limit hit (attempt {retry + 1}/{max_retries})"
                        )
                        print(f"   Waiting {wait_time} seconds before retry...")
        ...
        if len(formatted_rows) == 0:
            ...
            print("🔄 Attempting double verification with lenient rules...")
            print(
                "   (Allowing rows with 1-2 missing fields, but Plot No. is still required)"
            )
            print()
            ...
            try:
                ...
                if lenient_rows:
                    print(
                        f"✅ Lenient extraction found {len(lenient_rows)} rows with Plot No. information"
                    )
                    formatted_rows = lenient_rows
                else:
                    print(
                        "⚠️  Lenient extraction also found no rows with Plot No. information"
                    )
                    # Final fallback: Try with Gemini Vision Pro model
                    if current_model_name != "gemini-pro-vision":
                        print()
                        print(
                            "🔄 Attempting final fallback with Gemini Vision Pro model..."
                        )
                        try:
                            ...
                            vision_extracted_data = parse_json_response(
                            ...
                            if vision_rows:
                                print(
                                    f"✅ Vision Pro model found {len(vision_rows)} rows with Plot No. information"
                                )
                                formatted_rows = vision_rows
                            else:
                                print(
                                    "⚠️  Vision Pro model also found no rows with Plot No. information"
                                )
                        except Exception as vision_error:
                            print(
                                f"⚠️  Vision Pro model extraction failed: {vision_error}"
                            )
            except Exception as e:
                print(f"⚠️  Lenient extraction failed: {e}")
...
def process_batch(
    ...
    print(
        f"⏱️  Delay between files: {delay_between_files} seconds (to avoid rate limits)"
    )
    print("=" * 70)
    print()
    ...
    for i, pdf_file in enumerate(pdf_files, 1):
        ...
        if should_skip_pdf(pdf_file):
            ...
            print(
                f"⏭️  Skipping {filename} - output files already exist: {', '.join(existing_files)}"
            )
            print()
            ...
            try:
                if os.path.exists(csv_file):
                    ...
                    successful += 1
                    print(
                        f"   📂 Loaded {len(existing_rows)} rows from existing output"
                    )
                    print()
                else:
                    all_results[filename] = []
...
def process_directory(
    ...
    print(
        f"📋 Found {len(pdfs_to_process)} PDF(s) to process (skipped {skipped_count} with existing outputs)"
    )
    print()
    ...
    try:
        ...
        for directory in directories:
            dir_results = process_directory(
                directory, model, model_name, delay_between_files
            )
            ...
            if directory != directories[-1]:
                print(
                    f"⏳ Waiting {delay_between_files} seconds before next directory..."
                )
                print()
                time.sleep(delay_between_files)
