...
def format_chandra_line(line: str) -> Optional[str]:
    ...
    if "Model config" in line or "Qwen3VLConfig" in line:
        return None
    if line.strip().startswith(("_attn_", "architectures", "base_model", "dtype", 
                               "text_config", "vision_config", "hidden_size",
                               "num_attention", "intermediate_size", "vocab_size",
                               "max_position", "rope_", "tie_word", "use_cache",
                               "transformers_version", "video_token", "image_token",
                               "deepstack", "depth", "patch_size", "spatial_merge",
                               "temporal_patch", "vision_end", "vision_start")):
        return None
    ...
    if len(line) < 100:
        if any(keyword in line.lower() for keyword in 
               ["processing", "extracting", "saving", "output", "input", "method"]):
            # Clean up the message
            clean = line.strip()
            if not any(tech in clean for tech in ["Qwen3VL", "config", "safetensors.index"]):
                return f"ℹ️  {clean}"
...
def extract_text_from_pdf(pdf_path: str, use_structure: bool = True):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if 'CUDA_VISIBLE_DEVICES' not in env or env.get('CUDA_VISIBLE_DEVICES') == '':
            ...
            def print_formatted_output(pipe, pipe_name=""):
                ...
                try:
                    ...
                    for line in pipe:
                        ...
                        elif "loading weights" in line.lower() or "model" in line.lower():
                            with output_lock:
                                last_output_time[0] = time.time()
                except Exception as e:
                    pass  # Pipe closed
            ...
            def monitor_progress(process):
                ...
                while process.poll() is None:
                        ...
                        if proc:
                            try:
                                ...
                                if current_time - last_output_time[0] > 10:
                                    dots = (dots + 1) % 4
                                    status_dots = "." * dots
                                    sys.stdout.write(f"\r⏳ Loading model weights{status_dots} ({elapsed_min}m {elapsed_sec}s, CPU: {cpu_percent:.1f}%, RAM: {mem_mb:.0f}MB)     ")
                                    sys.stdout.flush()
                    ...
                    if elapsed > 600 and current_time - last_output_time[0] > 30:  # 10 min with no output
            ...
            if os.path.exists(json_file):
                print(f"📂 Loading extracted data from {os.path.basename(json_file)}...")
            ...
            if not full_text:
                ...
                if output_files:
                    ...
                    if returncode != 0:
                        error_msg += "Chandra OCR reported an error (see messages above)."
                    raise Exception(error_msg)
                else:
                    raise Exception("No output files generated by Chandra OCR. Check error messages above.")
...
def parse_table_rows(text: str, structured_data: Optional[List] = None) -> List[Dict[str, str]]:
    """
