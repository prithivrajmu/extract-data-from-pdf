...
def get_huggingface_ocr_models(api_key: Optional[str] = None, use_dynamic: bool = True) -> List[Dict[str, str]]:
    """
    ...
    if use_dynamic:
        try:
            from model_fetcher import fetch_ocr_models_from_huggingface, load_cache, clear_cache
            # Try to fetch models for API use
            models = fetch_ocr_models_from_huggingface(
                api_key=api_key,
                limit=30,
                for_api_use=True
            )
    ...
    # Fallback to static list of known API-compatible models
    return [
        {
