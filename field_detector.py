...
def parse_json_response(response_text: str) -> List[str]:
    ...
    logger.warning(
        "Could not parse field detection response. Preview: %s...", text[:200]
    )
    return []
...
def detect_fields_with_ai(
    ...
    if provider.lower() == "gemini":
        ...
        raise ValueError(
            f"Unsupported AI provider: {provider}. Use 'gemini' or 'deepseek'."
        )
    ...
    try:
        ...
        if pdf_file:
            try:
                ...
                logger.warning(
                    "Could not delete uploaded Gemini file: %s", delete_error
                )
...
def detect_fields_with_ocr(pdf_path: str, ocr_method: str = "chandra") -> List[str]:
    ...
    if ocr_method == "chandra":
        ...
        try:
            ...
            from examples.extract_ec_data_pytesseract import (
...
def _parse_headers_from_ocr_text(text: str) -> List[str]:
    ...
    if header_candidates:
        detected_fields = list(
            dict.fromkeys(header_candidates)
...
def detect_fields_from_pdf(
    ...
    if method == "ai":
        ...
        raise ValueError(
            f"Unknown detection method: {method}. Use 'auto', 'ai', or 'ocr'."
        )
...
def detect_fields_batch(
    ...
    if mode == "unified":
        ...
        return {"fields": unified_fields, "per_file": per_file_fields}

    else:
        raise ValueError(
            f"Unknown detection mode: {mode}. Use 'unified' or 'per_file'."
        )
