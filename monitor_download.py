...
def monitor_download():
    ...
    cache_dir = Path.home() / ".cache" / "huggingface" / "hub" / "models--datalab-to--chandra" / "blobs"
        ...
        while True:
            ...
            for inc_file in incomplete_files:
                ...
                if inc_file.name in last_size:
                    ...
                    if diff > 0:
                        print(f"\r⬇️  Downloading: {size_gb:.2f} GB | Speed: {speed_mbps:.1f} MB/s", end="", flush=True)
                    else:
                        print(f"\r⬇️  Downloading: {size_gb:.2f} GB | Waiting...", end="", flush=True)
                else:
