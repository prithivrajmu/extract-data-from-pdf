...
class ChandraOutputFormatter:
    ...
    def format_line(self, line: str) -> Optional[str]:
        ...
        if "Processing page" in line or "page" in line.lower() and any(x in line for x in ["1", "2", "3", "4", "5"]):
        ...
        if len(line) < 100 and not any(x in line for x in ["Qwen3VLConfig", "_attn_", "architectures", "base_model"]):
            # Only show if it's a meaningful message
            if any(keyword in line.lower() for keyword in ["processing", "extracting", "saving", "output", "input", "method"]):
