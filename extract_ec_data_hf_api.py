...
def chandra_ocr_hf_api(image: Image.Image, api_key: str, retries: int = 3) -> str:
    ...
    for attempt in range(retries):
        for payload_format_idx, payload in enumerate(payload_formats):
            try:
                print(f"   🌐 Sending to HF API (attempt {attempt + 1}/{retries}, format {payload_format_idx + 1})...")
                ...
                if response.status_code != 200:
                    ...
                    if response.status_code == 404:
                        ...
                        print("   💡 The Chandra model may not be available on Inference Providers")
                        print("   💡 Alternative: Use extract_ec_data_api.py with Datalab API")
                        if attempt < retries - 1:
                            continue
                        raise Exception(f"Model not available on Inference Providers API (404)")
                        ...
                        if attempt < retries - 1:
                            print(f"   ⚠️  Error {response.status_code}, trying next format...")
                            time.sleep(2)
                            continue
                ...
                if isinstance(result, dict):
                    ...
                    for key in ["generated_text", "text", "markdown", "output", "result"]:
                        if key in result:
                            return str(result[key])
                    # If list in result, try first item
                    if "generated_texts" in result and isinstance(result["generated_texts"], list):
...
def extract_data_from_pdf(pdf_path: str, api_key: Optional[str] = None) -> List[Dict[str, str]]:
    """Main function to extract data from a PDF file."""
    if not api_key:
        raise ValueError("HF_API_KEY is required. Get one from https://huggingface.co/settings/tokens")
...
def main():
    ...
    if api_key:
        ...
        print()
        print("   ⚠️  Note: If Chandra model is not available, use extract_ec_data_api.py")
